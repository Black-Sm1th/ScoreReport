...
{
    setsourceText(QString::fromLocal8Bit("评分依据：UCLA UCLS MRS 系统《Medicina 2020, 56(11), 569》\n版本时间：2020 年"));
}
...
UCLSMRSManager::ScoreResult UCLSMRSManager::evaluatePath(int macroFat, int microFat, int t2Signal, int arterialRatio1, int arterialIndex, int delayedIndex, int ader1, int arterialRatio2, int ader2)
{
    ...
    if (macroFat == HasMacroFat) {
        result.score = 1;
        result.diagnosis = QString::fromLocal8Bit("富脂肪AML");
        return result;
    }
    ...
    if (macroFat == NoMacroFat) {
        ...
        if (microFat == HasMicroFat) {
            if (t2Signal == HighSignal) {
                ...
                result.score = 5;
                result.diagnosis = QString::fromLocal8Bit("透明细胞癌");
            } else { // 低T2信号 (有微脂肪时没有"等"选项)
                // 无（宏观脂肪）-有（微脂肪）-低（T2信号强度）→ 1分（乏脂肪AML）
                result.score = 1;
                result.diagnosis = QString::fromLocal8Bit("乏脂肪AML");
            }
            return result;
        }
                ...
                if (arterialRatio1 == Greater) {
                    ...
                    if (arterialIndex == Greater) {
                        ...
                        if (delayedIndex == Greater) {
                            ...
                            result.score = 3;
                            result.diagnosis = QString::fromLocal8Bit("嗜酸细胞瘤");
                        } else {
                            // 无（宏观脂肪）-无（微脂肪）-高或等（T2信号强度）- 大于100（动脉强化比）-小于5（相对动脉增强比）-小于125（延迟增强指数）→ 4分（嫌色细胞癌）
                            result.score = 4;
                            result.diagnosis = QString::fromLocal8Bit("嫌色细胞癌");
                        }
                    }
                } else {
                    // 无（宏观脂肪）-无（微脂肪）-高或等（T2信号强度）- 小于等于100（动脉强化比）→ 4分（乳头状细胞癌）
                    result.score = 4;
                    result.diagnosis = QString::fromLocal8Bit("乳头状细胞癌");
                }
            } else {
                // 低T2信号强度
                if (arterialRatio1 == Greater) {
                    ...
                    if (ader1 == Greater) {
                        ...
                        result.score = 2;
                        result.diagnosis = QString::fromLocal8Bit("无脂肪AML");
                    } else {
...
void UCLSMRSManager::finishScore(int score, QString detailedDiagnosis)
{
    QString scoreText;
    switch(score) {
        case 1: scoreText = QString::fromLocal8Bit("肯定良性。"); break;
        case 2: scoreText = QString::fromLocal8Bit("可能良性。"); break;
        case 3: scoreText = QString::fromLocal8Bit("不确定。"); break;
        case 4: scoreText = QString::fromLocal8Bit("可能恶性。"); break;
        case 5: scoreText = QString::fromLocal8Bit("肯定ccRCC。"); break;
        default: scoreText = QString::fromLocal8Bit("未知。"); break;
    }
    QString title = QString::fromLocal8Bit("UCLS MRS评分：") + QString::number(score) + QString::fromLocal8Bit("分");
    QString result = "";
    resultText = title;
    if (detailedDiagnosis != "") {
        result = detailedDiagnosis + QString::fromLocal8Bit("，") + scoreText;
        resultText += "\n";
        resultText += result;
    }
    resultText += "\n";
